...
$folder = Join-Path $env:USERPROFILE "LovelyKidsPrintBridge"
$source = Join-Path $folder "LovelyKidsPrintBridgeStandalone.cs"
$exe    = Join-Path $folder "LovelyKidsPrintBridgeStandalone.exe"
...
namespace LovelyKidsPrintBridge
{
    ...
    {
        public static byte[] FromPng(byte[] imageBytes, int targetWidth, bool receipt = false)
        {
            ...
            using (Image src = Image.FromStream(input))
            {
                ...
                using (Bitmap bmp = new Bitmap(targetWidth, targetHeight, PixelFormat.Format24bppRgb))
                {
                    using (Graphics g = Graphics.FromImage(bmp))
                    {
                        ...
                        g.DrawImage(src, new Rectangle(receipt ? 0 : -24, 0, targetWidth, targetHeight),
                    ...
                    for (int y = 0; y < targetHeight; y++)
                    {
                        for (int xb = 0; xb < rowBytes; xb++)
                        {
                            ...
                            for (int bit = 0; bit < 8; bit++)
                            {
                                ...
                                if (lum < (receipt ? 210 : 128)) value = (byte)(value | (0x80 >> bit));
                            }
                    ...
                    using (MemoryStream output = new MemoryStream())
                    {
                        ...
                        if (receipt)
                        {
                            // Continuous receipt roll: normal paper feed, no label sensor command.
                        ...
                        {
                            // Preserve the calibrated QR-label behavior exactly.
        ...
        private static void Handle(TcpClient client)
        {
            using (client)
            {
                ...
                try
                {
                    ...
                    if (r.Method == "POST" &&
                        (uri.AbsolutePath == "/print-png" ||
                         uri.AbsolutePath == "/print-receipt-png"))
                    {
                        string printer = Config.GetPrinter();
                        ...
                        bool receipt = uri.AbsolutePath == "/print-receipt-png";
                        byte[] raw = Raster.FromPng(r.Body, width, receipt);
                        bool allOk = true;
...
Set-Content -Path $source -Value $code -Encoding UTF8

Write-Host "===== BUILD STANDALONE EXE =====" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "IMPORTANT: Do not run the new standalone EXE yet." -ForegroundColor Yellow
Write-Host "The old bridge is still using port 17858. Send the BUILD SUCCESS result to ChatGPT."
